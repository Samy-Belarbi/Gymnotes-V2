...
const removeExercice = (event) => {
    ...
    // FONCTION QUI REORGANISE TOUS LES NAMES DES INPUTS ET SELECT
...
const putMinusOneFor = (element, number) => {
    ...
    const lastCharacOfElement = element.getAttribute("for").charAt(element.getAttribute("for").length -1);
    const nameOfElementWithoutLastString = element.getAttribute("for").slice(0, -1);
    const numberOfElement = parseInt(lastCharacOfElement);
    
    if (numberOfElement > number) {
        element.setAttribute("for", nameOfElementWithoutLastString + (numberOfElement - 1));
    }
